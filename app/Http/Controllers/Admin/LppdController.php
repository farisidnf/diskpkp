            ...
            return DataTables::of($post)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (Auth::user()->role == 'admin'){
                        $btn = '<a href="'.route('admin.lppd.edit', $row->id).'" data-toggle="tooltip"  data-id="' . $row->id . '" data-original-title="Edit" class=" btn btn-success btn-sm "><i class="fa fa-edit"></i></a>';
...
//            'status' => 'required',
        ...
        if (!empty(($_FILES['upload_file']))) {
            ...
            $path = public_path() . '/uploads/lppd/';
            $path = $path . $newFileName;

            if ($fileSize <= 1048576) {
                if (move_uploaded_file($_FILES['upload_file']['tmp_name'], $path)) {
                    $upload_file = '/uploads/lppd/'.$newFileName;
                }
            }
        }
        ...
        return redirect()->route('admin.lppd.index')
